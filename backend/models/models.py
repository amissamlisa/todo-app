...
from sqlalchemy import Column, String, ForeignKey, Date, Float, DateTime, Enum
import enum
...
class Goals(Base):
  ...
  start_day: Mapped[Date] = mapped_column(nullable=False)
  completion_date: Mapped[Date] = mapped_column(nullable=False)
  status_against_goal: Mapped[str] = Column(String(200))
  ...
  task_creation_rule: Mapped[str | None] = Column(String(800))
...
class GoalsTasks(Base):
  ...
  estimated_time: Mapped[float] = mapped_column(Float(PRECISION, ASDECIMAL, SCALE), nullable=False)
