from sqlalchemy import String, ForeignKey, Date, Float, DateTime, Enum, Numeric
import enum
...
class Goals(Base):
  ...
  weekday_available_hours: Mapped[float] = mapped_column(Numeric(PRECISION, SCALE), nullable=False)
  weekends_available_hours: Mapped[float] = mapped_column(Numeric(PRECISION, SCALE), nullable=False)
  total_estimated_time: Mapped[float] = mapped_column(Numeric(PRECISION, SCALE), nullable=False)
...
class GoalsTasks(Base):
  ...
  estimated_time = mapped_column(Numeric(PRECISION, SCALE), nullable=False)
  created_at: Mapped[DateTime] = mapped_column(DateTime, nullable=False)